...
def compute_all_tasks_wages(
    ...
    end_date: date = Query(
        ...
        description="Compute all task untill this day.",
    ),
    payment_service: PaymentService = Security(initiate_payment_service, scopes=[]),
):
...
def get_all_task_wage_amount(
    unpaid: Optional[bool] = Query(None, description="To get amount of unpaid wages."),
    start_date: date = Query(date.today(), description="Get wages from this day."),
    end_date: date = Query(
        date.today() - timedelta(days=7),
        description="Get all wages till this day from start day.",
    ),
    payment_service: PaymentService = Security(initiate_payment_service, scopes=[]),
):

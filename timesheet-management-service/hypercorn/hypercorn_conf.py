...
use_tcp = os.getenv("USE_TCP", "True") == "True"
...
tcp_port = os.getenv("TCP_PORT", "8100")
...
if web_concurrency:
    ...
    if workers_max:
        ...
        assert use_web_concurrency > 0, "MAX_WORKERS and WORKERS_PER_CORE Must be non zero"  # type: ignore
...
access_log_format = os.getenv(
    "ACCESS_LOG_FORMAT", '%(h)s %(l)s %(l)s %(t)s "%(r)s" %(s)s %(b)s'
)
...
use_reloader = os.getenv("SHOULD_RELOAD", "False") == "True"
...
conf_data = {
    ...
    "backlog": backlog,
    "bind": bind,
    "insecure_bind": insecure_bind if use_tcp and use_ssl else None,  # type: ignore
    "quic_bind": quic_bind if use_quic_bind else None,  # type: ignore
